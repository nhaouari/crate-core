...
function CrateCore(id, options){
    ...
    this.id = this.app.overlay().communication.broadcast.uid
    ...
    // listen for incoming broadcast
      ...
      this.app.onBroadcast((id, message) => {
        console.log('I have received a message from peer', id, ':', message)   
        switch (message.type){
     ...
     // this.app.overlay().communication.broadcast._unicast.on('receive', (id, message) => {console.log(id, message)})  
      ...
      this.app.overlay().communication.broadcast.on('antiEntropy', (id, remoteVVwE, localVVwE) => {                  
                       ...
                       // console.log('before sending the response:', id, localVVwE, elements)
                        this.sendAntiEntropyResponse2(id, localVVwE, elements);
                  


        })
       
    /*  this.app.sendUnicast(this.app.getRandomNeighbourId(),this.app._networkManager._rpc._communication._causality)
    ...
    */
    ...
    window.sequence0 =  this.sequence;
    var self = this;
    // #A regular receive
    ...
    this.broadcast.on('updateSequence', function(counter){
        self.sequence._c=counter;
    });
    ...
    this.broadcast.on('antiEntropy', function(id, remoteVVwE, localVVwE){
        ...
        self.sendAntiEntropyResponse(id, localVVwE, elements);
...
CrateCore.prototype.sendAntiEntropyResponse =
    function(origin, causalityAtReceipt, messages){
        var id = GUID();
        // #1 metadata of the antientropy response
   
       
       var sent =this.app.sendUnicast(origin, new MAntiEntropyResponse(id, causalityAtReceipt, messages.length));

        var i = 0;
        while (sent && i < messages.length){
           sent=  this.app.sendUnicast(origin, new MAntiEntropyResponse(id, null, messages.length,messages[i]));
           ++i;
        };
    };

CrateCore.prototype.sendAntiEntropyResponse2 =
    function(origin, causalityAtReceipt, messages){
     //   console.log('sendAntyEntropyResponse2: ', messages)
        var id = GUID();
        // #1 metadata of the antientropy response
   
        var sent =this.app.overlay().communication.broadcast._unicast.send(origin, new MAntiEntropyResponse(id, causalityAtReceipt, messages.length));

        var i = 0;
        while (sent && i < messages.length){
           sent =this.app.overlay().communication.broadcast._unicast.send(origin, new MAntiEntropyResponse(id, null, messages.length,messages[i]));
           ++i;
        };
    };
...
CrateCore.prototype.insert = function(character, index){
    ...
    this.app.sendBroadcast(new MInsertOperation(ei, id._e))
...
CrateCore.prototype.remoteInsert = function(ei, origin){
   ...
   // 
    ...
    if (index >= 0 && origin){
        this.emit('remoteCaretMoved', {index: index, length: 0}, origin);
    };
};
...
CrateCore.prototype.sendPing = function(){
  ...
  if (store.get('myId').pseudo) {pseudo = store.get('myId').pseudo;} 

  this.broadcastCaret.send({type:'Mping',origin:id._e,pseudo:pseudo},
                             id,
                             null);
    return origin;
};
...
var id = this.broadcastCaret.causality.increment();  
this.broadcastCaret.send({type:'MTitleChanged',title:title},
                             id,
                             null);
    return origin;
};
