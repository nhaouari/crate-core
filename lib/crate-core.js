...
class CrateCore extends EventEmitter {
    ...
    constructor(id, options, data_comm, behaviours_comm) {
        ...
        this.id = this.options.editingSessionID
    ...
    insert(character, index) {
        ...
        this._communication.sendBroadcast(new MInsertOperation(pair, store.get('myId').id))
        this.setLastChangesTime()
        return pair;
    };
    ...
    remove(index) {
        ...
        this.sequence._c += 1;
        this._communication.sendBroadcast(new MRemoveOperation(i, store.get('myId').id))
    ...
    remoteInsert(pair, origin) {
        ...
        if (index >= 0 && origin && (!pair.antientropy)) {
